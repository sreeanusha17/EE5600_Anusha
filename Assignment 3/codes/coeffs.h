...
char c;
FILE *fp;
double x, mean, variance, sum=0.0, v_sum=0.0;
...
while(fscanf(fp,"%lf",&x)!=EOF)
{
sum+=x; // sum of all the values
}
mean = sum/count;
fclose(fp);
// finding the variance of the values
fp = fopen(str,"r");
while(fscanf(fp,"%lf",&x)!=EOF)
{
v_sum+= pow((x - mean),(2)); // sum of the deviation from the mean
}
variance=v_sum/count;
fclose(fp);
return variance;
}
